...
# Dataset
full_dataset = CentroidDataset(os.path.join(RESULTS_FOLDER, 'all_centroids_v5.csv'), IMAGE_FOLDER, MASKS_FOLDER)
...
def visualize_outputs(model_path, dataset, index=0, device='cuda',type=None):
    ...
    axs[1, 1].imshow(seg_target, cmap='gray')
    axs[1, 1].set_title("Predicted Mask")
    axs[1, 2].imshow(centroid_np, cmap='gray')
    ...
    plt.tight_layout()
    save_folder = os.path.join(RESULTS_FOLDER,"fft_0.9_model_v6_0.2")
    os.makedirs(save_folder,exist_ok=True)
    plt.savefig(os.path.join(save_folder,f"Model_v4_fft_{type}_{index}.png"))
    print("Updated")
...
model_path = os.path.join(BASE_DIR, "weights","weights_fft_new_microscopy_model_v6_0.2.pth")
