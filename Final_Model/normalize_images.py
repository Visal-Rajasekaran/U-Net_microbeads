...
OUTPUT_FOLDER = BASE_FOLDER / "dataset2" / "images_selected_normalized_3"
...
if __name__ == "__main__":
    ...
    for density, (num_clusters, cluster_range,_) in density_config.items():
        ...
        for img_path in images:
            ...
            print(np.median(normalized_img))
            #normalized_image[normalized_image > 150] = 255
            '''plt.imshow(normalized_img)
            plt.show()
            plt.close()'''
            cv2.imwrite(str(OUTPUT_FOLDER / density / Path(img_path).name), normalized_img)
