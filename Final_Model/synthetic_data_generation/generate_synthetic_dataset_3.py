...
SAMPLE_FOLDER = BASE_FOLDER / "synthetic_images_5_low_dense"
MASK_FOLDER = BASE_FOLDER / "masks_5_low_dense"
CSV_LOG = SAMPLE_FOLDER / "synthetic_centroids_5_low_dense.csv"
...
def create_bead_library(image, ref_brightness, edge_thresh_bead, bright_thresh_bead,edge_thresh_bg,bright_thresh_bg):
    ...
    print("Removed Bead Library")
    shutil.rmtree(BACKGROUND_FOLDER, ignore_errors=True)
    print("Removed Background Library")
    ...
    edge_thresh_bead = edge_thresh_bead
    bright_thresh_bead = bright_thresh_bead
    edge_thresh_bg = edge_thresh_bg
    bright_thresh_bg = bright_thresh_bg
...
def augment_image(img):
    ...
    # ---- Random brightness ----
    factor = np.random.uniform(0.3, 1.1)  # 0.7 = darker, 1.3 = brighter
    img = img * factor

    # ---- Random contrast ----
    #mean = img.mean()
    #contrast_factor = np.random.uniform(0.5, 0.7)
    #img = mean + contrast_factor * (img - mean)

    # ---- Add Gaussian noise ----
    if random.random() < 0.5:
        noise_std = np.random.uniform(3, 10)
        noise = np.random.normal(1, noise_std, img.shape)
        img += noise
    ...
    def add_color_noise(img, std_range=(5, 20)):
        img = img.astype(np.float32)
        std = np.random.uniform(*std_range)
        noise = np.random.normal(0, std, img.shape)  # (H, W, 3)
        img_noisy = img + noise
...
def generate_synthetic_image(image_id, num_clusters, cluster_size_range):
    ...
    with open(CSV_LOG, mode='a', newline='') as f:
        ...
        for bead_id, pos in enumerate(centroids, 1):
            ...
            cv2.circle(mask, (x, y), bead_radius-2, 255, thickness=-1)
...
if __name__ == "__main__":
    ...
    for density, (num_clusters, cluster_range, bead_size) in density_config.items():
        ...
        if density == "80x":
            ...
            print("Ok")
            apply_blur = True
            edge_thresh_bead, bright_thresh_bead,edge_thresh_bg,bright_thresh_bg = 50,130,10,190
