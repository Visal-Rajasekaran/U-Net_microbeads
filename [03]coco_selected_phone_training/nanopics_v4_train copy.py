...
IMAGE_FOLDER = os.path.join(BASE_DIR, "dataset", "augmented_images_v3")
...
# Dataset
full_dataset = CentroidDataset(os.path.join(RESULTS_FOLDER, 'all_centroids_copy.csv'), IMAGE_FOLDER, MASKS_FOLDER)
...
def visualize_outputs(model_path, dataset, index=0, device='cuda'):
    ...
    plt.tight_layout()
    plt.savefig(f"Model_v4_augmented_train_{index}.png")
...
for j in range(len(train_dataset)):
    visualize_outputs(model_path, train_dataset, index=j)
